...
const PaymentStatusPage: React.FC = () => {
    ...
    try {
      ...
      if (response.success && response.data) {
        ...
        if (retryCount < MAX_RETRIES) {
          ...
          setError(response.message || 'Không thể kiểm tra trạng thái thanh toán sau nhiều lần thử');
        }
      }
    } catch (err) {
  ...
  useEffect(() => {
    ...
    if (pollingCount >= MAX_POLLING && paymentInfo?.status === 'pending') {
      setError('Quá thời gian chờ xác nhận thanh toán. Vui lòng kiểm tra tài khoản của bạn.');
    }
  ...
  const handlePayAgain = () => {
    if (paymentInfo?.membership?.package_id?._id) {
      ...
      navigate(`/user/packages-register/${paymentInfo.membership.package_id._id}`);
    } else {
  ...
  if (isLoading) {
    return (
      <div className="container mx-auto px-4 py-8 text-center">
        ...
        <h2 className="mt-4 text-xl font-semibold">Đang kiểm tra trạng thái thanh toán...</h2>
        <p className="mt-2 text-gray-600">Vui lòng không đóng trang này khi đang xử lý</p>
        <div className="mt-4 h-2 max-w-md mx-auto rounded-full bg-gray-200">
          <div 
            className="h-full rounded-full bg-blue-500 transition-all duration-300"
            style={{ width: `${Math.min((pollingCount / MAX_POLLING) * 100, 100)}%` }}
          ></div>
        </div>
      </div>
    );
        ...
        <div className="mb-8 rounded-lg bg-white p-8 shadow-md dark:bg-gray-800">
          ...
          <div className="mb-6 text-center">
            {paymentInfo?.status === 'completed' ? (
              <>
                ...
                <h1 className="mt-4 text-2xl font-bold text-green-600">Thanh toán thành công!</h1>
              ...
              <>
                ...
                <h1 className="mt-4 text-2xl font-bold text-red-600">Thanh toán thất bại!</h1>
                <p className="mt-2 text-gray-600 dark:text-gray-400">
              ...
              <>
                ...
                <h1 className="mt-4 text-2xl font-bold text-yellow-600">Đang xử lý thanh toán...</h1>
                <p className="mt-2 text-gray-600 dark:text-gray-400">
              ...
              <div className="space-y-3">
                ...
                <div className="flex justify-between border-b pb-2">
                  ...
                  <span className={`font-medium ${
                    ...
                    paymentInfo.status === 'failed' ? 'text-red-600' : 'text-yellow-600'
                  }`}>
                    {paymentInfo.status === 'completed' ? 'Thành công' :
                     paymentInfo.status === 'failed' ? 'Thất bại' : 'Đang xử lý'}
                  </span>
                </div>
                  ...
                  <div className="mt-2 grid grid-cols-2 gap-4">
                    <div>
                      ...
                      <p className="font-medium text-gray-900 dark:text-white">
                        {new Date(paymentInfo.membership.start_date).toLocaleDateString('vi-VN')}
                      </p>
                    </div>
                    <div>
                      <p className="text-sm text-gray-600 dark:text-gray-400">Ngày kết thúc:</p>
                      <p className="font-medium text-gray-900 dark:text-white">
                        {new Date(paymentInfo.membership.end_date).toLocaleDateString('vi-VN')}
                      </p>
                    </div>
                  </div>
                </div>
          ...
          {paymentInfo?.status === 'failed' && (
            <div className="mt-6 rounded-lg bg-gray-50 p-4 dark:bg-gray-700">
              <h3 className="font-medium text-gray-900 dark:text-white">Cần hỗ trợ?</h3>
              <p className="mt-2 text-sm text-gray-600 dark:text-gray-400">
                Nếu bạn đã bị trừ tiền nhưng trạng thái hiển thị thất bại, vui lòng liên hệ với 
                bộ phận hỗ trợ khách hàng qua: <span className="font-medium text-blue-600 dark:text-blue-400">hotro@fittlife.vn</span> hoặc 
                hotline: <span className="font-medium text-blue-600 dark:text-blue-400">1900 xxxx</span>
              </p>
            </div>
          )}
        </div>
