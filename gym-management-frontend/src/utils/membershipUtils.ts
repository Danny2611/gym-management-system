...
export const calculateMembershipRemaining = (membership: Membership): {
  ...
  if (endDate) {
    ...
    switch (membership.status) {
      case 'active':
        ...
        remaining_days = Math.max(0, Math.floor(diffTime / (1000 * 60 * 60 * 24)));
        ...
        if (startDate) {
          ...
          if (totalDuration > 0) {
            ...
            remaining_percent = Math.max(0, Math.min(100, Math.round((remainingDuration / totalDuration) * 100)));
          } else {
            remaining_percent = 0;
          }
        }
